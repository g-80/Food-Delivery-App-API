...
public class CartItemsRepository : BaseRepo, ICartItemsRepository
{
    ...
    public async Task<CartItem?> GetCartItemById(int id)
    {
        ...
        {
            return await connection.QuerySingleOrDefaultAsync<CartItem>(sql, parameters);
        }
        ;
    }
    ...
    {
        var parameters = new { dto.CartId, dto.RequestedItem.ItemId, dto.RequestedItem.Quantity, dto.UnitPrice, dto.Subtotal };
        const string sql = @"
    ...
    {
        var parameters = new { cartId, itemId, quantity, newSubtotal };
